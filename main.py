#from sonnet_gen import get_sonnet as generate_sonnet
...
def generate_sonnet(debugging=False):
    tot = 0
    count = 0
    out_quote = []
...
def get_sonnet(event):
    # Use the imported function with the new name
    pydom["div#sonnet"].html = f"{generate_sonnet(debugging=False)}"